		...
		<view class="content">
			...
			<view class="rightContainer">
				<scroll-view scroll-y="true" class="contentScroll">
					...
					<view class="contentItem" v-for="item in categoryObj.subCateList" :key="item.name">
						<image :src="item.wapBannerUrl" mode="" lazy-load="true"></image>
						<text>{{item.name}}</text>
					</view>
				</scroll-view>
			</view>
...
<script>
	...
	export default {
		...
		computed:{
			navList(){
				const arr = this.categoryDatas.map((item)=>{
					return item.name
				})
				return arr;
			},
